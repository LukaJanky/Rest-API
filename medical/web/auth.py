...
def login():
    if request.method == 'POST':
        ...
        if user:
            ...
            flash('There is no user with this specific email!', category='error')
...
def sign_up():
    if request.method == 'POST':
        ...
        if len(email) < 4:
            ...
            flash('Role length must be greater than 1 character!', category='error')
            ...
            new_role = Role(role_name=role, user_role_id=current_user.id)
